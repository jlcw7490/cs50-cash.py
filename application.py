...
def buy():
    """Buy shares of stock"""
     # reached route via POST (as by submitting a form via POST)
    if request.method == "POST":

        # Store the dictionary returned from the search in a variable
        symbol_entered = lookup(request.form.get("symbol"))
        ...
        if symbol_entered == None:
            return apology("invalid symbol", 400)
        elif not shares_entered.isdigit() or int(shares_entered) < 1:
        ...
        value = symbol_entered["price"] * int(shares_entered)
        ...
        # If the user can afford the purchase, proceed
            ...
            username=db.execute("SELECT username FROM users WHERE id = :uid", uid=int(session['user_id']))[0]["username"],
            symbol=symbol_entered['symbol'], price=symbol_entered['price'], shares_entered=request.form.get('shares'))
            ...
            username=db.execute("SELECT username FROM users WHERE id = :uid", uid=int(session['user_id']))[0]["username"],
            symbol=symbol_entered['symbol'], shares_entered=request.form.get('shares'))
...
def quote():
    ...
    #if the symbol is invalid, return apology
        if look == None:
          return apology("invalid symbol", 400)
...
def sell():
    ...
    if request.method == "POST":
        ...
        # If the symbol searched or number of shares is invalid, return apology
        if not request.form.get("symbol") or symbol_entered == None:
            return apology("you must provide a stock", 400)
        elif not shares_entered or not shares_entered.isdigit() or int(shares_entered) < 1 or int(shares_entered) > int(user_shares):
        ...
        # If everything checks, proceed with sale
            ...
            username=username, symbol=symbol_entered['symbol'], price=symbol_entered['price'], shares_entered=request.form.get('shares'))

            # If the user is selling all the shares, remove the stock from the user's portfolio
            if int(user_shares) == int(shares_entered):
            ...
            elif int(user_shares) > int(shares_entered):
                db.execute("UPDATE portfolio SET shares = :shares WHERE username = :username and symbol = :symbol",
                            shares=shares_entered, username=username, symbol=request.form.get("symbol"))
